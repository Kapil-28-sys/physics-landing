...
const GeoBg = ({ isDark }) => {
  const lineColor       = isDark ? "rgba(201,168,76,0.07)" : "rgba(139,90,20,0.06)";
  const lineColorBright = isDark ? "rgba(201,168,76,0.16)" : "rgba(139,90,20,0.11)";
  return (
    <div style={{ position:"absolute", inset:0, overflow:"hidden", pointerEvents:"none", zIndex:0 }}>
      <style>{`
        @keyframes grid-pan { 0%{transform:translate(0,0)} 100%{transform:translate(40px,40px)} }
        @keyframes grid-shimmer { 0%,100%{opacity:0.55} 50%{opacity:1} }
        .grid-pan { animation:grid-pan 18s linear infinite }
        .grid-shimmer { animation:grid-shimmer 6s ease-in-out infinite }
      `}</style>
      <svg width="calc(100% + 80px)" height="calc(100% + 80px)" style={{position:"absolute",top:-40,left:-40}} className="grid-pan">
        <defs><pattern id="grid-base" x="0" y="0" width="40" height="40" patternUnits="userSpaceOnUse">
      ...
      <svg width="calc(100% + 240px)" height="calc(100% + 240px)" style={{position:"absolute",top:-120,left:-120}}>
        <defs><pattern id="grid-large" x="0" y="0" width="120" height="120" patternUnits="userSpaceOnUse">
        ...
        <rect className="grid-pan grid-shimmer" style={{animationDirection:"reverse",animationDuration:"30s"}} width="100%" height="100%" fill="url(#grid-large)"/>
      </svg>
    </div>
  );
};
...
const TeacherPhoto = ({ theme }) => {
  ...
  return (
    <div style={{position:"relative",width:"100%",height:"100%"}}>
      <div style={{position:"absolute",top:"50%",left:"50%",transform:"translate(-50%,-50%)",fontFamily:"'Cormorant Garamond',serif",fontSize:"clamp(52px,8vw,96px)",fontWeight:700,fontStyle:"italic",color:isDark?"rgba(201,168,76,0.04)":"rgba(139,90,20,0.06)",whiteSpace:"nowrap",letterSpacing:"-0.02em",pointerEvents:"none",zIndex:0,userSelect:"none",lineHeight:1}}>Physics Pro</div>
      ...
      <div style={{position:"absolute",bottom:8,left:0,zIndex:2,display:"flex",alignItems:"center",gap:10,background:isDark?"rgba(11,17,32,0.94)":"rgba(255,255,255,0.97)",backdropFilter:"blur(16px)",border:isDark?"1px solid rgba(201,168,76,0.3)":"1px solid rgba(201,168,76,0.22)",borderRadius:14,padding:"10px 16px",boxShadow:isDark?"0 8px 32px rgba(0,0,0,0.45)":"0 8px 32px rgba(0,0,0,0.12)"}}>
...
const Modal = ({ open, onClose, title, children, T }) => {
  ...
  if (!open) return null;
  return (
    <div onClick={onClose} style={{position:"fixed",inset:0,zIndex:200,background:"rgba(0,0,0,0.55)",backdropFilter:"blur(6px)",display:"flex",alignItems:"flex-end",justifyContent:"center",padding:"0"}}>
      <div onClick={e=>e.stopPropagation()} style={{background:T.bg,border:`1px solid ${T.border}`,borderRadius:"24px 24px 0 0",width:"100%",maxWidth:560,maxHeight:"80vh",overflowY:"auto",padding:"32px 28px 48px",boxShadow:"0 -16px 64px rgba(0,0,0,0.3)"}}>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:24}}>
          <h3 style={{fontFamily:"'Cormorant Garamond',serif",fontSize:22,fontWeight:700,color:T.text}}>{title}</h3>
          <button onClick={onClose} style={{width:36,height:36,borderRadius:"50%",background:T.surface,border:`1px solid ${T.border}`,display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",color:T.textDim}}><X size={16}/></button>
        </div>
        <div style={{color:T.textMid,fontSize:14,lineHeight:1.8}}>{children}</div>
      </div>
    </div>
  );
};
...
export default function PhysicsProLanding() {
  ...
  const T = {
    ...
    surface: isDark ? "rgba(255,255,255,0.035)" : "#ffffff",
    border: isDark ? "rgba(255,255,255,0.08)" : "rgba(0,0,0,0.08)",
    borderMed: isDark ? "rgba(255,255,255,0.12)" : "rgba(0,0,0,0.13)",
    text: isDark ? "#f0f4ff" : "#1a1a2e",
    textMid: isDark ? "rgba(180,195,230,0.75)" : "rgba(40,40,70,0.7)",
    textDim: isDark ? "rgba(180,195,230,0.42)" : "rgba(40,40,70,0.46)",
    textFaint: isDark ? "rgba(180,195,230,0.24)" : "rgba(40,40,70,0.3)",
    navBg: isDark ? "rgba(11,17,32,0.97)" : "rgba(250,248,243,0.97)",
    navBorder: isDark ? "rgba(255,255,255,0.06)" : "rgba(0,0,0,0.07)",
    navScrollBorder: isDark ? "rgba(201,168,76,0.2)" : "rgba(201,168,76,0.32)",
    gold: "#c9a84c",
    goldDim: isDark ? "rgba(201,168,76,0.12)" : "rgba(201,168,76,0.1)",
    goldBorder: isDark ? "rgba(201,168,76,0.3)" : "rgba(201,168,76,0.36)",
    teal: "#0d9488",
    ...
    shadow: isDark ? "0 12px 40px rgba(0,0,0,0.45)" : "0 12px 40px rgba(0,0,0,0.12)",
    shadowCard: isDark ? "0 2px 16px rgba(0,0,0,0.32)" : "0 2px 16px rgba(0,0,0,0.07)",
    shadowGold: "0 6px 28px rgba(201,168,76,0.38)",
  };
  ...
  const goTo = useCallback((dir) => {
    ...
    setTimeout(()=>{setSlideIndex((i)=>(i+dir+testimonials.length)%testimonials.length); setSliding(false);}, 380);
  }, [sliding, testimonials.length]);

  useEffect(() => {
    autoRef.current = setInterval(()=>goTo(1), 4800);
    return ()=>clearInterval(autoRef.current);
  }, [goTo]);

  const resetAuto = () => { clearInterval(autoRef.current); autoRef.current=setInterval(()=>goTo(1),4800); };
  ...
  const whyTrust = [
    ...
    "Thousands of Successful Students","Strong Focus on Conceptual Understanding",
    "Personalised Attention for Every Learner","Weak & High-Performing Students Both",
    ...
    <div style={{fontFamily:"'DM Sans', sans-serif",background:T.bg,color:T.text,overflowX:"hidden",minHeight:"100vh",transition:"background 0.3s ease, color 0.3s ease"}}>
      <style>{`
        ...
        *{box-sizing:border-box;margin:0;padding:0;}
        ...
        .btn-ghost{cursor:pointer;transition:all 0.18s;background:transparent;border:none;font-family:inherit;color:inherit;}
        ...
        @keyframes pulse-ring{0%,100%{opacity:0.3;transform:scale(1);}50%{opacity:0.55;transform:scale(1.04);}}
        @keyframes hl-fade{0%{opacity:0;transform:translateY(10px);}100%{opacity:1;transform:translateY(0);}}
        .hl-animate{animation:hl-fade 0.55s cubic-bezier(0.22,1,0.36,1) forwards;}
        ...
        .anim-in-right{animation:slideInRight 0.38s cubic-bezier(0.4,0,0.2,1) forwards;}
        .anim-in-left{animation:slideInLeft 0.38s cubic-bezier(0.4,0,0.2,1) forwards;}
        ...
        .footer-link:hover{color:${T.gold};}
        .footer-contact-chip{display:flex;align-items:center;gap:10px;padding:10px 16px;border-radius:12px;background:${T.surface};border:1px solid ${T.border};cursor:pointer;transition:all 0.18s;text-decoration:none;}
        .footer-contact-chip:hover{border-color:${T.goldBorder};transform:translateY(-1px);}
        .divider-rule{height:1px;background:linear-gradient(90deg,transparent,${T.divider},transparent);margin:0 48px;}
        ...
        @media(max-width:768px){
          ...
          .divider-rule{margin:0 20px!important;}
          ...
          .footer-links-row{flex-direction:column;gap:24px!important;}
      ...
      {footerLinks.filter(l=>l.content).map((l)=>(
        <Modal key={l.label} open={modal===l.label} onClose={()=>setModal(null)} title={l.label} T={T}>
          {l.content}
        </Modal>
      ))}
      ...
      <nav ref={navRef} className={`desktop-nav${scrolled?" scrolled":""}`}>
        ...
        <div style={{display:"flex",gap:10,alignItems:"center"}}>
          ...
          <button className="btn" onClick={openMail} style={{fontSize:13,fontWeight:600,color:"#0b1120",background:"linear-gradient(135deg,#c9a84c,#e8c96a)",border:"none",borderRadius:9,padding:"9px 20px",display:"flex",alignItems:"center",gap:7,boxShadow:T.shadowGold}}>
      ...
      <div className="page-wrap">
            ...
            <div className="hero-text" style={{flex:1,minWidth:0,maxWidth:560,paddingBottom:64}}>
              ...
              <div {...fadeIn("hero",0.3)} style={{display:"flex",gap:14,flexWrap:"wrap",marginBottom:48}}>
                <button className="btn" onClick={()=>openWA("Hi Sunira ma'am! I want to enrol in your Physics classes.")}
                  style={{fontSize:13.5,fontWeight:700,color:"#0b1120",background:"linear-gradient(135deg,#c9a84c,#e8c96a)",border:"none",borderRadius:10,padding:"13px 28px",display:"inline-flex",alignItems:"center",gap:9,boxShadow:T.shadowGold,letterSpacing:"0.04em",textTransform:"uppercase"}}>
                  Enrol on WhatsApp
                </button>
        ...
        <div className="divider-rule" style={{background:`linear-gradient(90deg,transparent,${isDark?"rgba(201,168,76,0.18)":"rgba(201,168,76,0.25)"},transparent)`}}/>
            ...
            <div className="about-grid" style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:56,alignItems:"start"}}>
              <div>
                <div {...fadeIn("about",0)}>
                  ...
                  <h2 style={{fontFamily:"'Cormorant Garamond',serif",fontSize:"clamp(28px,4vw,42px)",fontWeight:700,letterSpacing:"-0.02em",lineHeight:1.12,color:T.text,marginBottom:6}}>Meet Sunira Agarwal</h2>
        ...
        <section id="testimonials" ref={registerRef("testimonials")} data-observe className="section-pad" style={{padding:"72px 48px"}}>
          ...
          <div style={{display:"flex",justifyContent:"center",gap:8,marginTop:20}}>
            {testimonials.map((_,i)=>(
              <button key={i} className="btn" onClick={()=>{setSlideDir(i>slideIndex?1:-1);setSliding(true);setTimeout(()=>{setSlideIndex(i);setSliding(false);},380);resetAuto();}} style={{width:i===slideIndex?24:8,height:8,borderRadius:4,background:i===slideIndex?T.gold:T.borderMed,border:"none",transition:"all 0.3s",padding:0}}/>
            ))}
          </div>
        </section>
        ...
        <div className="divider-rule" style={{background:`linear-gradient(90deg,transparent,${isDark?"rgba(201,168,76,0.15)":"rgba(201,168,76,0.22)"},transparent)`}}/>
        ...
        {/* ── FOOTER ── */}
        <footer className="footer-inner" style={{padding:"48px 48px 40px"}}>

          {/* Top row: brand + contact chips */}
          <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",flexWrap:"wrap",gap:32,marginBottom:40}} className="footer-links-row">

            {/* Brand */}
            <div style={{maxWidth:340}}>
              <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:14}}>
                <div style={{width:36,height:36,borderRadius:10,background:"linear-gradient(135deg,#c9a84c,#e8c96a)",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,boxShadow:"0 4px 16px rgba(201,168,76,0.3)"}}><Atom size={17} color="#0b1120"/></div>
                <div>
                  <div style={{fontFamily:"'Cormorant Garamond',serif",fontSize:17,fontWeight:700,color:T.text,lineHeight:1.2}}>Sunira Agarwal</div>
                  <div style={{fontSize:10.5,color:T.gold,letterSpacing:"0.1em",textTransform:"uppercase",marginTop:2}}>Physics · 30+ Years · DPS Faridabad</div>
                </div>
              </div>
              ...
              <div className="footer-contact-row" style={{display:"flex",gap:10,flexWrap:"wrap"}}>
                <div onClick={()=>openWA()} className="footer-contact-chip">
                  <div style={{width:28,height:28,borderRadius:8,background:"rgba(37,211,102,0.1)",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}><MessageSquare size={14} color={T.emerald}/></div>
                  <span style={{fontSize:12.5,color:T.textMid,fontWeight:500}}>WhatsApp Us</span>
                  <ExternalLink size={11} color={T.textFaint} style={{marginLeft:2}}/>
                </div>
                <div onClick={openMail} className="footer-contact-chip">
                  <div style={{width:28,height:28,borderRadius:8,background:T.goldDim,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}><Mail size={14} color={T.gold}/></div>
                  <span style={{fontSize:12.5,color:T.textMid,fontWeight:500}}>Send Email</span>
                  <ExternalLink size={11} color={T.textFaint} style={{marginLeft:2}}/>
                </div>
              </div>
            </div>
            ...
            <div style={{display:"flex",gap:48,flexWrap:"wrap"}}>
              <div>
                <div style={{fontSize:11,color:T.textFaint,textTransform:"uppercase",letterSpacing:"0.12em",fontWeight:600,marginBottom:16}}>Navigation</div>
                <div style={{display:"flex",flexDirection:"column",gap:10}}>
                  {navLinks.map((n)=>(
                    <button key={n.label} className="footer-link" onClick={()=>scrollToSection(n)} style={{textAlign:"left",fontSize:13,color:T.textDim,padding:0}}>
                      {n.label}
                    </button>
                  ))}
                </div>
              ...
              <div>
                <div style={{fontSize:11,color:T.textFaint,textTransform:"uppercase",letterSpacing:"0.12em",fontWeight:600,marginBottom:16}}>Legal</div>
                <div style={{display:"flex",flexDirection:"column",gap:10}}>
                  {footerLinks.map((l)=>(
                    <button key={l.label} className="footer-link" onClick={()=>l.action?l.action():setModal(l.label)} style={{textAlign:"left",fontSize:13,color:T.textDim,padding:0}}>
                      {l.label}
                    </button>
                  ))}
                </div>
          ...
          {/* Theme toggle row */}
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:14,padding:"18px 0",borderTop:`1px solid ${T.divider}`,borderBottom:`1px solid ${T.divider}`,marginBottom:20}}>
            <div style={{display:"flex",alignItems:"center",gap:10}}>
              <Shield size={13} color={T.textFaint}/>
              <span style={{fontSize:12,color:T.textFaint}}>Your information is never shared with third parties.</span>
            </div>
            <button className="btn" onClick={()=>setTheme(isDark?"light":"dark")} style={{display:"flex",alignItems:"center",gap:8,fontSize:12.5,color:T.textDim,background:T.surface,border:`1px solid ${T.border}`,borderRadius:9,padding:"8px 16px"}}>
          ...
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:12}}>
            <span style={{fontSize:12,color:T.textFaint}}>© 2026 Sunira Agarwal. All rights reserved.</span>
            <span style={{fontSize:12,color:T.textFaint}}>Made with ❤️ for Physics learners across India</span>
          </div>
        </footer>
      </div>
    </div>
